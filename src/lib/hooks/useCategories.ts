import { useEffect, useCallback, useMemo, useRef } from 'react';
import { usePathname } from 'next/navigation';
import { useCategoriesState } from '@/lib/contexts';
import type { Category } from '@/lib/types/meme';

interface UseCategoriesOptions {
  limit?: number;
}

interface UseCategoriesReturn {
  categories: Category[];
  loading: boolean;
  error: string | null;
  refetch: () => void;
}

let categoriesListInFlight: Promise<void> | null = null;

export const useCategories = (options: UseCategoriesOptions = {}): UseCategoriesReturn => {
  const { state, setCategories, setLoading, setError, refetch: refetchState } = useCategoriesState();
  const pathname = usePathname();
  const prevPathnameRef = useRef<string | null>(null);

  const limit = useMemo(() => options.limit || 50, [options.limit]);

  const fetchCategories = useCallback(
    async (opts?: { silent?: boolean }) => {
      const silent = opts?.silent ?? false;

      const run = async () => {
        try {
          if (!silent) {
            setLoading(true);
          }
          setError(null);

          const params = new URLSearchParams({
            sort_by: 'name',
            sort_order: 'asc',
            limit: limit.toString(),
          });

          const response = await fetch(`/api/categories?${params}`);

          if (!response.ok) {
            throw new Error(`Failed to fetch categories: ${response.statusText}`);
          }

          const data = await response.json();

          if (data.success) {
            setCategories(data.categories);
          } else {
            throw new Error(data.error || 'Failed to fetch categories');
          }
        } catch (err) {
          setError(err instanceof Error ? err.message : 'An error occurred');
          console.error('Error fetching categories:', err);
        }
      };

      if (categoriesListInFlight) {
        await categoriesListInFlight;
        return;
      }
      categoriesListInFlight = run().finally(() => {
        categoriesListInFlight = null;
      });
      await categoriesListInFlight;
    },
    [limit, setCategories, setLoading, setError]
  );

  // Always reconcile with the server on mount so filters match admin changes (sessionStorage can be stale).
  useEffect(() => {
    const silent = state.categories.length > 0 && state.isInitialized;
    void fetchCategories({ silent });
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional: mount-only bootstrap
  }, []);

  useEffect(() => {
    const prev = prevPathnameRef.current;
    prevPathnameRef.current = pathname;
    if (pathname === '/memes' && prev !== null && prev !== '/memes') {
      void fetchCategories({ silent: true });
    }
  }, [pathname, fetchCategories]);

  useEffect(() => {
    const onVis = () => {
      if (document.visibilityState === 'visible') {
        void fetchCategories({ silent: true });
      }
    };
    document.addEventListener('visibilitychange', onVis);
    return () => document.removeEventListener('visibilitychange', onVis);
  }, [fetchCategories]);

  const refetch = useCallback(() => {
    refetchState();
    void fetchCategories({ silent: false });
  }, [refetchState, fetchCategories]);

  return {
    categories: state.categories,
    loading: state.loading,
    error: state.error,
    refetch,
  };
};
